...
export const registerSchema = z
  .object({
    ...
    middleName: z
      .string()
      .optional()
      .refine((val) => !val || val.length >= 2, 'Tên đệm phải có ít nhất 2 ký tự')
      .refine((val) => !val || /^[a-zA-ZÀ-ỹ\s]+$/.test(val), 'Tên đệm chỉ được chứa chữ cái'),
    lastName: z
      .string()
...
export const phoneSchema = z.object({
  phoneNumber: z
    ...
    .regex(
      /^(0[3|5|7|8|9])\d{8}$/,
      'Số điện thoại phải bắt đầu bằng 03, 05, 07, 08, hoặc 09 và có 10 số',
    ),
});
