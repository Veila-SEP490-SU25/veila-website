...
export const TransactionCard = ({ transaction, onUpdate }: TransactionCardProps) => {
  return (
    <Card>
      <div className="flex items-center justify-between p-4 rounded-lg relative">
        {transaction.type === TransactionType.WITHDRAW &&
          transaction.status === TransactionStatus.PENDING && (
            <WithdrawProcessDialog transaction={transaction} onUpdate={onUpdate}>
        ...
        <div className="flex items-center space-x-4">
          <div
            className={`p-2 rounded-full ${
              [TransactionType.TRANSFER, TransactionType.WITHDRAW].includes(transaction.type)
                ? 'bg-red-100'
                : 'bg-green-100'
            }`}
          >
